...
- **Robust Glyph Grid Font Rendering**: Reimplemented and optimized the glyph cell rendering by permanently subscribing to `ContainerContentChanging` during page load (preventing a race condition when the items source is loaded). During font changes and TTC navigation, we explicitly iterate over all realized containers to immediately apply the new font, ensuring the typeface updates family-wide across all visible cells.

### 🐛 Fixed
- **TTC Cache Isolation & Sub-Font Rendering**: Fixed a critical caching bug in WinUI 3's XAML text engine where private fonts loaded via URIs (`ms-appx:///` or `ms-appdata:///`) cached by the base file path only, causing sub-font navigation inside TrueType Collections (`.ttc`) to load only the first font face's typeface. By including the `fontIndex` in the cache uniqueness validation, each sub-font face gets a unique path and separate base URI, completely bypassing the WinUI 3 caching bug and rendering all sub-font glyphs perfectly.
- **TTC UI Navigation & Font Style/Weight Selection**: Fixed a major bug where navigating TTC sub-fonts using the UI navigation buttons (`PrevFontButton` / `NextFontButton`) failed to update the visual style (regular/bold/italic) of the glyph cells because the family name was identical and the grid items had no `FontWeight` or `FontStyle` bindings. Added null-safe `LoadedFontWeight` and `LoadedFontStyle` properties in the ViewModel, bound the `GlyphGrid` and its cell `TextBlock`s to them, and registered the `CurrentFont` property change listener in `GlyphsTabPage` to guarantee updates whenever a sub-font is changed.
- **Glyph Grid First-Load Blank & Scroll Issue**: Resolved the virtualization issue where visible cells on the first screen did not load the correct font until scrolled. By using robust XAML-level ElementName bindings to propagate the FontFamily, FontWeight, and FontStyle directly to cell text blocks, we eliminated fragile code-behind layout-pass visual tree traversals entirely.
...
- **Optimized Glyph Render Pipeline**: Restored high-efficiency asynchronous UI virtualization in the GridView. By combining permanent layout pass event callbacks with targeted synchronous viewport updates, we avoid heavy bindings or full visual tree rebuilds during rapid scrolling and font transitions.
...
### 🔄 Changed
- **Settings layout**: Preview section groups Quick View, default preview text/background, Editable Preview, and Waterfall; expandable items keep an activity toggle and nested sub-settings (Windows Settings-style).
- **Glyphs tab layout**: Narrowed the Unicode block sidebar and reduced left padding so the glyph grid uses more horizontal space.
...
- **FontViewerPage Binding Crashes**: Resolved startup freeze/crash after the navigation refactor by moving header, install, and settings-dependent visibility bindings into `FontViewerViewModel` with proper change notification, replacing nullable `CurrentFont`/`SelectedGlyph` compiled-binding paths with null-safe properties, and restoring the initial waterfall build on font load.
- **Settings Page Crash on Open**: Restored missing `BoolToVisibility` converter resources on `SettingsPage` (previously supplied only by `MainWindow` before the shell/page split), which caused an unhandled XAML parse exception when navigating to Settings.
- **Settings Back Navigation & Title Bar**: Fixed losing an open font when leaving Settings (singleton `FontViewerViewModel`, `NavigationCacheMode="Required"`, skip reload on `NavigationMode.Back`). Back button now appears only on Settings and returns to the font viewer underneath. Fixed `DisplayArea.FindAll()` `InvalidCastException` during window placement. Corrected backdrop ComboBox labels (Mica / Mica Alt / Acrylic / Solid) to match stored backdrop values.
- **Settings Live-Update Crash**: Fixed `NullReferenceException` in `UpdateFontDisplay` when changing settings while `FontViewerPage` is off-screen — subscribe to `SettingsService.Changed` only while the page is active, guard UI updates when the visual tree is detached, and stop removing the private GDI font session on page unload.
- **Quick View Default Background**: Restored the system-default card appearance after the page split by re-applying theme brushes from app resources instead of `ClearValue`. When the app theme is Light or Windows light, Quick View "System default" now matches the application page background (transparent on Mica/Acrylic, `ApplicationPageBackgroundThemeBrush` on solid backdrop); dark themes keep the elevated card fill for contrast.
